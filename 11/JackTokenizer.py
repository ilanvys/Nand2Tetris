...
class JackTokenizer:
    ...
    def is_sequence_end(self) -> bool:
        """Checks if the current token is the end of the current sequence
        Returns:
            true if the token is the end of the sequence, false otherwise
        """
    ...
    def peek(self) -> str:
        """Checks the next token without advancing the token sequence.
        """
        prev_char_index = self.char_index
        prev_curr_token = self.curr_token
        prev_curr_token_type = self.curr_token_type
        if self.has_more_tokens():
            self.advance()

        peeked_token = self.curr_token
        self.char_index = prev_char_index
        self.curr_token = prev_curr_token
        self.curr_token_type = prev_curr_token_type
